    ...
    <div className="bg-inherit">
      <div className="max-w-7xl mx-auto pt-3 pb-8 sm:px-6 lg:px-8">
        <div className="max-w-3xl mx-auto divide-y-2 divide-white/20">
          <h2 className="text-center text-2xl sm:text-3xl font-semibold text-primary">{title}</h2>
    ...
    <div className="pt-6">
      <dt className="text-lg">
        <button className="text-left w-full flex items-start text-white" onClick={toggleOpen}>
          ...
          <span className="sm:flex sm:items-center font-medium text-inherit"><span className='mr-2'>{item.heading}</span>{item.isOpenSource ? <OpenSourceBadge /> : <></>}</span>
        </button>
      </dt>
